...
export interface AstroAdapterFeatures {
	/**
	 * Creates and edge function that will communicate with the Astro middleware
	 */
...
A set of features that changes the output of the emitted files. When an adapter opt ins these features, they will get additional
information in specific hooks.

### `functionPerRoute`

This is a feature that is enabled only when using SSR. By default, Astro emits a single `entry.mjs` file, which is responsible to emit the rendered page at each request.

When `functionPerRoute` is `true`, Astro will create a file for each route defined in the project. 
Each file emitted will render only one page. The pages will be emitted inside a `dist/pages/` directory, and the emitted files will keep the same file paths of the `src/pages` directory.
...
Then, consume the hook [`astro:build:ssr`](/en/reference/integrations-reference/#astrobuildssr), which will give you an `entryPoints`, an object that maps a page route to the physical file emitted after the build.
  ...
  return {
    ...
    hooks: {
      ...
      'astro:build:ssr': ({ middlewareEntryPoint }) => {
          // remember to check if this property exits, it will be `undefined` if the adapter doesn't opt in the feature
